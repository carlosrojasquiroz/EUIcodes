function dist = getDist_cns(p,m,g_a)
    g_a=g_a';
    ...
    for i = 1:p.naa      
        for j = 1:p.nzz
             TM((i-1)*p.nzz+j,(indbelow(j,i)-1)*p.nzz+1:(indbelow(j,i))*p.nzz) = wbelow(j,i)*m.Pz(j,:);  
             TM((i-1)*p.nzz+j,(indabove(j,i)-1)*p.nzz+1:(indabove(j,i))*p.nzz) = wabove(j,i)*m.Pz(j,:);  
        end
    end
